...
REM ===========================================================================
REM  Kawaii-Vault - fresh delete + clone + run
REM
REM  HOW TO USE:
REM    1. COPY this file OUT of the Kawaii-Vault folder and into your Documents
REM       folder  (e.g. %USERPROFILE%\Documents).
REM    2. Double-click it there.
REM
REM  It deletes the "Kawaii-Vault" folder sitting NEXT TO this .bat and downloads
REM  a brand-new copy from the 'main' branch, then installs and launches it.
REM
REM  Do NOT run it from inside the Kawaii-Vault folder - it would try to delete
REM  the folder it is running from. (There is a safety check below.)
REM ===========================================================================

set "BASE=%~dp0"
set "TARGET=%BASE%Kawaii-Vault"
...
if exist "%BASE%package.json" (
  echo.
  echo ERROR: This .bat appears to be INSIDE the Kawaii-Vault folder.
...
echo.
echo Launching Kawaii-Vault - the FIRST run compiles the backend and is slow
