<div align="center">

# 📜 Terms of Service — CyberVault

**Last updated: June 20, 2026**

</div>

---

> # ✨ Only made possible by **feris** ✨
> ### 💜 CyberVault was **only made possible by feris's imagination**, brought to life with **Claude's work**. 💜
> ### Contact: 👉 **[https://mez.ink/ferisooo](https://mez.ink/ferisooo)** 👈

---

## 1. Acceptance of these Terms

By downloading, installing, running, copying, forking, or modifying CyberVault
(the **"Software"** or **"App"**), you agree to these Terms of Service (the
**"Terms"**). If you do not agree, do not use the Software.

These Terms apply to everyone who uses the Software, including people who fork
or create modified versions of it.

---

## 2. Forking & Modification — Permission Granted

You are **welcome and explicitly allowed** to fork, modify, redistribute, and
build your own versions of CyberVault, **on one simple condition**:

> ### ✅ You must clearly credit **feris's imagination** and **Claude's work**.

This credit must be visible to your users — for example in your README, your
About screen, your documentation, or your app's credits — wherever it is
reasonable for someone to find it. A clear line such as:

> *"Based on CyberVault — original concept and imagination by feris
> ([https://mez.ink/ferisooo](https://mez.ink/ferisooo)), built with the help of
> Claude."*

is sufficient.

As long as you provide that credit, you may use, copy, modify, merge, publish,
distribute, and create derivative works freely. This permission operates
alongside the project [`LICENSE`](./LICENSE) (MIT). Where the MIT License and
these Terms differ, the **attribution requirement above is a condition of all
permitted use**.

You may **not** misrepresent the origin of the Software, remove existing
credits to feris or Claude, or claim that you created the original concept.

---

## 3. The Software is Provided "As Is"

CyberVault is provided **"AS IS"** and **"AS AVAILABLE"**, without warranty of
any kind, express or implied, including but not limited to the warranties of
merchantability, fitness for a particular purpose, title, and
non-infringement.

feris and any contributors make **no guarantee** that the Software will be
secure, error-free, uninterrupted, or that it will meet your requirements.

---

## 4. Your Responsibility & Assumption of Risk

CyberVault is a **local, self-hosted** tool that you run on your own device.
You are solely responsible for:

- The files you choose to store, encrypt, import, or share using the Software.
- **Remembering your password.** There is **no master key, no backdoor, and no
  recovery mechanism.** If you forget or lose your password, your data is
  **permanently unrecoverable.**
- Keeping your own backups of any important data.
- Ensuring your use of the Software is **lawful** in your jurisdiction.
- Securing the device(s) on which you run the Software, including any phone
  access or network features you choose to enable.

You use the Software **entirely at your own risk.**

---

## 5. Lawful Use Only

You agree **not** to use CyberVault to store, transmit, or process any content
that is illegal, or to engage in any unlawful activity. feris does not host,
monitor, access, or control any data you handle with the Software, and bears
**no responsibility** for how you choose to use it. **You alone** are
responsible and liable for your use of the Software and for any content you
process with it.

---

## 6. Limitation of Liability

To the **maximum extent permitted by applicable law**, in no event shall
feris, Claude, Anthropic, or any contributor be liable for any claim, damages,
or other liability — whether in an action of contract, tort, or otherwise —
arising from, out of, or in connection with the Software or its use, including
but not limited to:

- loss of data, files, or access to your vault;
- data corruption, security breaches, or unauthorized access;
- lost profits, business interruption, or any direct, indirect, incidental,
  special, consequential, or punitive damages.

This limitation applies even if feris has been advised of the possibility of
such damages. Some jurisdictions do not allow certain limitations, so parts of
this section may not apply to you; in that case liability is limited to the
**smallest amount permitted by law**.

---

## 7. No Support Obligation

CyberVault is offered freely. feris is **under no obligation** to provide
support, updates, bug fixes, or continued availability of the Software, and may
change, discontinue, or stop maintaining it at any time without notice.

---

## 8. Third-Party Components

CyberVault is built on open-source technologies (such as Tauri, React, Rust,
and others) which are governed by their own licenses. Your use of those
components is subject to their respective terms.

---

## 9. Indemnification

You agree to indemnify and hold harmless feris and any contributors from and
against any claims, liabilities, damages, losses, and expenses (including
reasonable legal fees) arising out of or in any way connected with your use of
the Software or your violation of these Terms.

---

## 10. Changes to these Terms

These Terms may be updated from time to time. The "Last updated" date at the
top reflects the latest version. Continued use of the Software after changes
means you accept the revised Terms.

---

## 11. Severability

If any provision of these Terms is held to be invalid or unenforceable, the
remaining provisions will continue in full force and effect.

---

<div align="center">

### 💜 CyberVault — concept & imagination by **feris**, built with **Claude**. 💜
### 👉 [https://mez.ink/ferisooo](https://mez.ink/ferisooo) 👈

</div>
