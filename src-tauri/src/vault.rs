...
/// file is ever re-encrypted with the same DEK.
pub fn encrypt_file_data(key: &[u8], _salt: &[u8], _file_id: &str, plaintext: &[u8]) -> Result<Vec<u8>, String> {
...
/// Decrypt file data using AES-256-GCM with chunked decryption.
pub fn decrypt_file_data(key: &[u8], _salt: &[u8], _file_id: &str, ciphertext: &[u8], plaintext_size: u64) -> Result<Vec<u8>, String> {
    ...
    while offset < ciphertext.len() {
        ...
        let decrypted = cipher.decrypt(nonce, &ciphertext[offset..offset + chunk_ct_size])
...
fn wrap_file_key(kek: &[u8], dek: &[u8]) -> Result<String, String> {
    let cipher = Aes256Gcm::new_from_slice(kek)
...
fn unwrap_file_key(kek: &[u8], wrapped_hex: &str) -> Result<Zeroizing<Vec<u8>>, String> {
    ...
    let plaintext = cipher.decrypt(nonce, &wrapped[12..])
...
fn resolve_file_key(kek: &[u8], wrapped_dek: Option<&str>) -> Result<Zeroizing<Vec<u8>>, String> {
    match wrapped_dek {
        Some(hex) => unwrap_file_key(kek, hex),
...
pub fn read_decrypted_range(
    ...
    for chunk_idx in first_chunk..=last_chunk {
        ...
        let decrypted = cipher.decrypt(nonce, &chunk_buf[ENC_NONCE_SIZE..])
...
fn bundle_read_file(path: &Path, vault: &VaultData, file_id: &str, encryption_key: Option<&[u8]>) -> Result<Vec<u8>, String> {
    ...
    if let (Some(kek), Some(salt_hex)) = (encryption_key, &vault.security.encryption_salt) {
        ...
        let file_key = resolve_file_key(kek, wrapped_dek.as_deref())?;
        decrypt_file_data(&file_key, &salt, file_id, &buf, plaintext_size)
    } else {
        Ok(buf)
    }
}
...
impl VaultManager {
    ...
    fn import_one(
        ...
        let (blob_data, wrapped_dek) = if let Some((ref salt, ref kek)) = ctx.encryption {
            ...
            let wrapped = wrap_file_key(kek, &dek)?;

            // Encrypt file data with per-file DEK
            let encrypted = encrypt_file_data(&dek, salt, &file_id, &data)?;
    ...
    pub fn get_file_stream_info(&self, file_id: &str) -> Result<FileStreamInfo, String> {
        ...
        let (enc_key, enc_salt) = if let Some(ref salt_hex) = vault.security.encryption_salt {
            ...
            let file_key = resolve_file_key(&kek, target_wrapped_dek.as_deref())?;
            let salt = hex::decode(salt_hex).map_err(|e| format!("Invalid salt: {}", e))?;
